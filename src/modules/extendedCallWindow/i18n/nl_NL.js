module.exports = {
    arrCounter: {
        resetTexts: {
            counter: 'Inzetvoorstelteller',
            highlight: 'Inzetvoorstelomranding',
            selection: 'Voertuigselectie',
            counter_highlight: 'Inzetvoorstelteller / -omranding',
            counter_selection: 'Inzetvoorstelteller en voertuigselectie',
            highlight_selection: 'Inzetvoorstelomranding en voertuigselectie',
            counter_highlight_selection:
                'Inzetvoorstelteller / -omranding en voertuigselectie',
        },
        reset: '{text} terugzetten',
    },
    arrHover: {
        reset: 'De voertuigselectie zal vooraf worden gereset!',
        titles: {
            set:
                'Zo vaak wordt dit voertuigtype geselecteerd, als je op deze inzetvoorstel drukt',
            attribute: 'Die Kombination, die in dieser Zeile beschrieben wird',
            free: 'Zo vaak is dit voertuigtype beschikbaar',
            max:
                'Zo vaak kan je dit inzetvoorstel selecteren, totdat dit voertuigtype niet meer beschikbaar is',
        },
        arrSpecs: {
            fire: 'Tankautospuit',
            elw: 'Officier van Dienst Brandweer',
            elw2: 'Hoofdofficier van Dienst - Brandweer',
            elw3: 'Commandovoertuig of haakarmbak',
            dlk: 'Redvoertuig',
            arff: 'Crashtender',
            elw: 'Airport Fire Officer / On Scene Commander',
            rtw: 'Ambulance',
            kdow: 'Officier van Dienst Geneeskunde',
            nef: 'MMT-Auto of Lifeliner',
            rth: 'Lifeliner',
            nef: 'MMT-Auto',
            ktw: 'Zorgambulance',
            fly: 'Rapid Responder',
            ambulance: 'Ambulance of Rapid Responder',
            kdow: 'OVD-G of OVDG-RR',
            fly: 'OVDG-RR of Rapid Responder',
            rw: 'Hulpverleningsvoertuig',
            gwl2wasser: 'Slangenwagen / Watertankwagen of Watertransportsysteem-haakarmbak',
            gwl2wasser: 'Alleen Slangenwagen / Watertankwagen',
            abl2wasser: 'Watertransportsysteem-haakarmbak',
            gw: 'Waterongevallenvoertuig',
            boot: 'WOA (Water ongeval aanhanger)',
            gwa: 'Adembeschermingsvoertuig of haakarmbak',
            ab: 'Adembeschermingshaakarmbak',
            gwmesstechnik: 'Dienstbus Verkenningseenheid Brandweer',
            gwgefahrgut: 'Adviseur Gevaarlijke Stoffen',
            mtw: 'Dienstauto / Dienstbus brandweer',
            fustw: 'Politie Noodhulp',
            police: 'Dienstmotor Noodhulp',
            fustw: 'Politie Noodhulp of Dienstmotor Noodhulp',
            polizeihubschrauber: 'Politiehelikopter',
            hondengeleider: 'Hondengeleider',
            ovd: 'Officier van Dienst - Politie',
            lebefkw: 'ME Commandovoertuig',
            grukw: 'ME Groepsvoertuig',
            spokesman: 'Voorlichters',
            at: 'AT-Commandant',
            at: 'AT-Operator',
            at: 'AT-Materiaalwagen',
        },
    },
    enhancedMissingVehicles: {
        vehicle: 'Voertuigtype',
        missing: 'Missende voertuigen',
        driving: 'Aanrijdend',
        total: 'Nog nodig',
        selected: 'Geselecteerd',
        vehiclesByRequirement: {
            [/^Tankautospuit(en)$/]: [0, 1, 6, 7, 8, 9, 12, 14, 15, 17, 50],
            [/^Redvoertuig(en)$/]: [2, 18],
            [/^Officieren(en) van Dienst - Brandweer$/]: [3, 19],
            [/^Hulpverleningsvoertuig(en)$/]: [4, 51],
            [/^Adembeschermingsvoertuig of haakarmbak$/]: [5, 27],
            [/^Slangenwagen, watertankwagen of gelijkwaardige haakarmbak$/]: [10, 29, 34, 45],
            [/^Hoofd Officier(en) van Dienst$/]: [19],
            [/^Commandowagen(s)$/]: [31, 32],
            [/^Verkenningseenh(eden|eid)$/]: [11],
            [/^Adviseur(s) Gevaarlijke Stoffen$/]: [24],
            [/^Waterongevallenvoertuig(en) \/ Oppervlaktereddingsteam(s)$/]: [33, 49, 50],
            [/^Waterongevallenaanhanger(s)$/]: [36],
            [/^Crashtender(s)$/]: [41, 42, 43],
            [/^AFO\/OSC$/]: [44],
            [/^Voorlichter(s)$/]: [56],
            [/^Noodhulpeenhe(id|den)$/]: [22, 25, 46, 47, 48],
            [/^Officier(en) van Dienst - Politie$/]: [35],
            [/^Politiehelikopter$/]: [28],
            [/^Hondengeleider(s)$/]: [47, 48],
            [/^ME Commandovoertuig(en)$/]: [39],
            [/^ME Groepsvoertuig(en)$/]: [40],
            [/^AT Commandant(en)$/]: [53],
            [/^AT Materiaalwagen(s)$/]: [55],
            [/^AT Operator(s)$/]: [54],
            [/^Ambulance(s)$/]: [16, 52, 57],
        },
    },
    tailoredTabs: {
        defaultTabs: [
            {
                name: 'TS',
                vehicleTypes: [0, 1, 6, 7, 8, 9, 12, 14, 15, 17, 50],
            },
            {
                name: 'Brandweer overig',
                vehicleTypes: [2, 18, 3, 19, 4, 51, 5, 27, 10, 29, 34, 45, 31, 32, 11, 24, 56],
            },
            {
                name: 'Waterongevallen',
                vehicleTypes: [33, 49, 50, 36],
            },
            {
                name: 'Vliegveld',
                vehicleTypes: [41, 42, 43, 44],
            },
            {
                name: 'Ambulance',
                vehicleTypes: [16, 52, 57, 30, 23, 37, 38],
            },
            {
                name: 'Politie Noodhulp',
                vehicleTypes: [22, 25, 46],
            },
            {
                name: 'Politie overig',
                vehicleTypes: [35, 28, 47, 48, 39, 40, 53, 54, 55],
            },
        ],
        allTab: 'Alle',
        occupiedTab: 'Dooralarmeren',
    },
    hideVehicleList: {
        show: 'Toon voertuigenlijst',
        hide: 'Verberg voertuigenlijst',
    },
};
