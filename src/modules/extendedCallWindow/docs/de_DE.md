...
<code><v-pre>{{type}}</v-pre></code> - wird durch die MissionTypeID ersetzt, dies ist eine Nummer, die jeden Einsatztyp im Spiel kennzeichnet, der Mülleimerbrand als beispiel hat die 0, der Containerbrand die 1, und so weiter

<code><v-pre>{{credits}}</v-pre></code> - wird durch die durchschnittlichen Credits des Einsatzes ersetzt.
