...
(async function() {
    ...
    function getBuildingMarker(htmlData) {
        ...
        for (var i = 0; i < htmlData.length; i++) {
            if (htmlData[i].innerHTML.includes("buildingMarkerAdd")) {
                const regexDatabase = htmlData[i].innerHTML,
    ...
    $('a[href*="profile"]').each(async function() {
        const $this = $(this);
        const candidatureId = +$this.attr('href').replace(/\D+/g, '');
        var totalCredits = 0;
        $this.parent().parent("tr").attr("id", `table_${ candidatureId }`);
        await $.get($this.attr('href'), function(data) {
            totalCredits = parseInt($('h1', data).siblings('br')[0].nextSibling.textContent.replace(/\D+/g, ''));
            const userBuildings = getBuildingMarker($("script", data));
            $this
                .after(`<p style="color:limegreen;display:inline;margin-left:2em">${ totalCredits.toLocaleString() } verdiente Credits</p>
            ...
            for (var i = 0; i < userBuildings.length; i++) {
                var e = userBuildings[i],
                    buildingHtml = `<a href="/buildings/${ e.id }" class="lightbox-open">${ e.name }</a>`;
                if (i != userBuildings.length - 1) $('#buildings_' + candidatureId).append(buildingHtml + '<span style="color:red"> - </span>');
                else $('#buildings_' + candidatureId).append(buildingHtml);
            }
            if ($('.profile_avatar', data)[0]) {
                $('#profileInfo_' + candidatureId).append($('.profile_avatar', data)[0].outerHTML);
            }
            if ($('#profile_text_photo', data)[0]) {
                $('#profileInfo_' + candidatureId).append($('#profile_text_photo', data).text().trim());
            }
            if ((!$('.profile_avatar', data)[0] || $('.profile_avatar', data).length === 0) && (!$('#profile_text_photo', data)[0] || !$('#profile_text_photo', data).text().trim())) {
                $('#profileInfo_' + candidatureId).append("Keine Profilinformationen verfügbar.");
            }
        });
    });

    $("body").on("click", ".glyphicon-info-sign", function() {
        var toggleId = $(this).attr('id').replace('info_', '');
        $('#buildings_' + toggleId).toggleClass('hidden');
    });

    $("body").on("click", ".glyphicon-user", function() {
        var toggleId = $(this).attr('id').replace('profile_', '');
