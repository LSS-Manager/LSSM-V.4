<template>
    <div>
        <ul>
            <li v-for="[module, {translations}] in noMapkitModules" :key="module">
                <AutoLink :item="{link: `/${lang}/modules/${module}/`, text: translations[lang].name}"/>
            </li>
        </ul>
        <div>
            {{settingsText}}:
...
const noMapkitModules = computed(() => Object.entries(themeData.value.variables.modules).filter(([, module]) => module.registration.noMapkit));
</script>
